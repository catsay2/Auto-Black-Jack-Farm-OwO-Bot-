...
async def on_ready():
    print(
        f"{Fore.LIGHTRED_EX} > Cat OwO BlackJack Farm v2 Connected To:{Style.RESET_ALL}",
...
async def start(ctx):
    ...
    await ctx.send(f"✅ Cat OwO BlackJack Farm Running. Starting balance: **__{balance:,}__** cowoncy.")
...
async def stop(ctx):
    global farming_active, data
    if not farming_active:
        return await ctx.send("⏹ Cat OwO BlackJack Worker is not running.")
    farming_active = False
    data["timer_end"] = None
    save_data(data)
...
async def status(ctx):
    ...
    cfg = load_config()
    text = (
        f"📊 **__Cat OwO BLACKJACK FARM STATUS__**\n\n"
